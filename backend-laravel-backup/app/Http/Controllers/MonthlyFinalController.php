...
public function show(string $final_id)
{
    ...
    // Build category details from final_category_averages (NOT from internal reviews)
    $categoryDetails = [];
    foreach ($monthlyFinal->final_category_averages ?? [] as $catId => $avgRating) {
        $category = $categories->get($catId);
        
        // Get the final rating data (if exists in final_category_ratings)
        $finalRating = ($monthlyFinal->final_category_ratings ?? [])[$catId] ?? null;
        ...
        // Only include final comment/evidence if present in final_category_ratings
        if ($finalRating) {
