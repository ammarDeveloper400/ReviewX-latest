...
class UserController extends Controller
{
    ...
    public function destroy(string $user_id)
    {
        ...
        if ($user_id === $currentUser->id) {
            return response()->json(['detail' => 'Cannot delete your own account'], 400);
        }
        ...
        if ($linkedEmployeeId) {
            $employee = Employee::find($linkedEmployeeId);
            if ($employee) {
                $linkedEmployeeName = $employee->full_name;
            }
        }

        // Delete user (this will cascade delete the linked employee)
        $user->delete();

        // Log audit entry
        AuditLog::log($currentUser->id, 'DELETE_USER', 'user', $user_id, [
            'deleted_by' => $currentUser->email,
            'user_email' => $userEmail,
            'user_role' => $user->role,
            'linked_employee_id' => $linkedEmployeeId,
            'linked_employee_name' => $linkedEmployeeName,
        ]);

        return response()->json([
            'message' => 'User account deleted successfully',
            'linked_employee_deleted' => $linkedEmployeeId !== null,
        ]);
    }
}
